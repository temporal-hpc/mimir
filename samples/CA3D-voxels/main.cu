...
static void print_usage(const char* prog)
{
    ...
    printf("  --shading MODE     how living voxels are lit (default unlit):\n");
    printf("                       unlit         opaque, unlit cubes\n");
    printf("                       phong         cubes lit by the scene light (better 3D shape)\n");
    printf("                       path-traced   ray-traced boxes (needs an RT-capable GPU)\n");
    printf("                      (a voxel is a cube, so mimir's --geometry axis does not apply\n");
    printf("                       here; aliases: --render-path, --light-model, old values\n");
    printf("                       flat/none/phong/path-tracing)\n");
    ...
    printf("                     living cells are visible. Applies to the chosen shading.\n");
...
// Living voxels are cubes by construction, so the Geometry axis does not apply here: --shading is
// the only draw knob this sample exposes (opacity is separate, --opacity).
...
int main(int argc, char **argv){
    ...
    for (int i = 8; i < argc; ++i)
    {
        ...
        // --render-path / --light-model are the pre-split spellings, kept as aliases. A voxel has
        // no geometry choice, so they only ever carried a shading here.
        else if ((a == "--shading" || a == "--render-path" || a == "--light-model") && i + 1 < argc)
        {
            std::string m = argv[++i];
            if      (m == "unlit" || m == "flat" || m == "none")  shading = Shading::Unlit;
            else if (m == "phong" || m == "impostor")             shading = Shading::Phong;
            else if (m == "path-traced" || m == "path-tracing" || m == "pt")
                                                                  shading = Shading::PathTraced;
            else { fprintf(stderr, "Unknown shading '%s' (unlit|phong|path-traced)\n", m.c_str());
                   return EXIT_FAILURE; }
        }
    ...
    // itself (opts.show_hud, F2 toggles it) -- this sample links no ImGui and writes no GUI code.
    {
        ...
        const char* shading_name = (shading == Shading::Phong) ? "phong"
            : (shading == Shading::PathTraced) ? "path-traced" : "unlit";
        printf("Scene : grid %ld^3 (%llu voxels), shading=%s, opacity=%.2f", n, fine, shading_name, opacity);
