...
__global__ void kernelSeedColors(float3 *seed_colors, curandState *states, size_t n)
{
...
void setSeedColors(Setup *setup){
    kernelSeedColors<<<setup->seeds_grid, setup->seeds_block>>>(setup->gpu_seed_colors, setup->r_device, setup->S);
    cudaDeviceSynchronize();
}

__global__ void kernelWriteColors(float4 *vd_colors, float3 *seed_colors, int *vd, size_t N)
{
    int tidx = blockDim.x * blockIdx.x + threadIdx.x;
    int tidy = blockDim.y * blockIdx.y + threadIdx.y;
    int tid = tidy * N + tidx;
    if (tidx < N && tidy < N)
    {
        auto c = seed_colors[vd[tid]];
        vd_colors[tid] = {c.x, c.y, c.z, 1.f};
    }
}
