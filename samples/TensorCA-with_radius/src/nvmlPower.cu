...
*/
void GPUPowerBegin(int N, int us, int comptype, std::string strr) {
    GPU_SAMPLE_US = us;
    int i;
    // Initialize nvml.
    nvmlResult = nvmlInit();
    ...
    GPUfilename = std::string("GPUPOWER-") + std::to_string(N) + "-" + std::to_string(comptype) + "-" + strr + std::string(".dat");
