...
BenchmarkResult runExperiment(BenchmarkInput input, NBodyParams params)
{
    ...
    // Cheapest possible particles: unlit 2D point sprites (no geometry shader, no lighting, no
    // custom depth) -- the same cost class as datoviz's flat disc markers in samples/nbody-datoviz.
    // The library defaults (Phong + Impostor) would render lit ray-sphere impostors instead.
    options.shading  = Shading::Unlit;
    options.geometry = Geometry::Sprite;
    ...
    if (input.display)
    {
        ...
        ViewDescription desc
        {
            ...
            // Under Geometry::Sprite markers are native point sprites sized in PIXELS
            // (params.point_size is a world-space unit for the sphere geometries and does
